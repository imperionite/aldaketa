...
viz_choice = st.radio(
    ...
    ["Visualization 1: IoT Water Quality Dashboard", "Visualization 2: Blockchain-Verified Monitoring"]
)
...
if viz_choice == "Visualization 1: IoT Water Quality Dashboard":
    ...
    **Mode:** IoT Water Quality Dashboard  
    ...
    agg_option = st.sidebar.selectbox(
        "Aggregate Data By",
        options=["Raw (No aggregation)", "Minute", "Hour"],
        index=0
    )
    ...
    def aggregate_df(df, agg_option):
        if agg_option == "Raw (No aggregation)":
            ...
            return df.groupby(['data_type', pd.Grouper(freq='T')])['numeric_value'].mean().reset_index().set_index('timestamp')
        elif agg_option == "Hour":
            return df.groupby(['data_type', pd.Grouper(freq='H')])['numeric_value'].mean().reset_index().set_index('timestamp')
        else:
            return df
    ...
    st.dataframe(desc_df.set_index("Parameter").style.format("{:.2f}", subset=["Mean", "Std", "Min", "25%", "50%", "75%", "Max"]))

    st.subheader("💡 Automated Insights")
    flat_params = []
    for param in selected_params:
        unique_vals = filtered_df[filtered_df['data_type'] == param]['numeric_value'].nunique()
        if unique_vals == 1:
    ...
    for param in selected_params:
        ...
        if param in flat_params:
            st.info(f"**{param}** remained constant at {param_df.iloc[0]:.2f} throughout the session.")
        else:
            st.markdown(
    ...
    for param in selected_params:
        ...
        if smoothing:
            chart_df['smoothed'] = chart_df['numeric_value'].rolling(window=3, min_periods=1).mean()
        ...
        chart_df['anomaly'] = np.abs(chart_df['numeric_value'] - rolling_mean) > 2 * rolling_std

        base = alt.Chart(chart_df.reset_index()).mark_line(point=True).encode(
            x=alt.X('timestamp:T', title='Timestamp'),
            y=alt.Y('numeric_value:Q', title=param),
            tooltip=['timestamp:T', 'numeric_value:Q'] + (['smoothed:Q'] if smoothing else [])
        ).properties(title=f"{param} Over Time", height=300)

        chart = base
        if smoothing:
            smooth_line = alt.Chart(chart_df.reset_index()).mark_line(color='red').encode(
                x='timestamp:T',
                y='smoothed:Q'
            )
            chart += smooth_line

        anomaly_points = alt.Chart(chart_df.reset_index()).mark_point(color='orange', size=60, filled=True).encode(
            x='timestamp:T',
            y='numeric_value:Q',
        ...
        })).mark_rule(strokeDash=[4,2], color='orange').encode(
            y='threshold:Q',
            tooltip=['label:N', 'threshold:Q']
        )
    ...
    **Mode:** Blockchain-Verified Monitoring  
    ...
    def load_data():
        ...
        df.set_index("timestamp", inplace=True)
        df["ledger_hash"] = df.apply(lambda row: hashlib.sha256(
            f"{row['sensor_id']}{row['data_type']}{row['numeric_value']}{row.name}".encode()).hexdigest(), axis=1)

        def wqi(row):
            score = 100
            if row["data_type"] == "pH" and (row["numeric_value"] < 6.5 or row["numeric_value"] > 8.5): score -= 30
            if row["data_type"] == "Turbidity (NTU)" and row["numeric_value"] > 5: score -= 25
            if row["data_type"] == "Temperature (°C)" and (row["numeric_value"] < 5 or row["numeric_value"] > 35): score -= 10
            if row["data_type"] == "Conductivity (µS/cm)" and row["numeric_value"] > 1500: score -= 15
            if row["data_type"] == "ClO2 MS1 (mg/L)" and not (0.2 <= row["numeric_value"] <= 0.8): score -= 20
            return max(score, 0)
    ...
    sensors = st.sidebar.multiselect("Sensor(s)", sensor_choices, default=sensor_choices)
    types = st.sidebar.multiselect("Data Type(s)", type_choices, default=type_choices)
    date_range = st.sidebar.date_input("Date Range", [df.index.min().date(), df.index.max().date()])

    filtered = df[(df.index.date >= date_range[0]) & (df.index.date <= date_range[1])]
    filtered = filtered[filtered["sensor_id"].isin(sensors) & filtered["data_type"].isin(types)]

    st.subheader("🌟 Water Quality Index (WQI) Overview")
    st.markdown("WQI summarizes overall water quality. 90+ is Good, 70-89 Moderate, below 70 Poor.")
    ...
    st.caption("Gauge shows average WQI for selected data. Status is shown in the number and title.")

    st.subheader("🔗 Blockchain Integrity Trace")
    filtered = filtered.copy()
    filtered["block_id"] = (filtered.reset_index(drop=True).index // 10) + 1
    filtered["validation_node"] = filtered["block_id"].apply(lambda x: f"Node-{x%3+1}")
    st.dataframe(filtered.head(10)[["sensor_id", "data_type", "numeric_value", "ledger_hash", "block_id", "validation_node"]])
    ...
    st.markdown("Simulate a data breach: pH values artificially increased by 20%. Blockchain hashes reveal tampering.")
    ...
    tampered["tampered_hash"] = tampered.apply(lambda row: hashlib.sha256(
        f"{row['sensor_id']}{row['data_type']}{row['numeric_value']}{row.name}".encode()).hexdigest(), axis=1)
    ...
    if tampered["tampered"].any():
        ...
        st.dataframe(tampered[tampered["tampered"]][["sensor_id", "data_type", "numeric_value", "ledger_hash", "tampered_hash"]])
    else:
    ...
    threshold = st.slider("Threshold value", float(filtered["numeric_value"].min()), float(filtered["numeric_value"].max()), step=0.5)
    alert_df = filtered[(filtered["data_type"] == alert_type) & (filtered["numeric_value"] > threshold)]
    st.markdown(f"**Alert: {len(alert_df)} readings exceed {threshold} in {alert_type}**")
    if not alert_df.empty:
        st.dataframe(alert_df[["sensor_id", "numeric_value", "WQI_score"]].head(10))

    st.subheader("⏱️ Sensor Uptime & Activity")
    activity_df = filtered.reset_index().groupby(['sensor_id', pd.Grouper(key='timestamp', freq='h')]).size().reset_index(name='readings')
    fig_activity = px.bar(
        activity_df,
        x="timestamp",
        y="readings",
